...
class EvaluationRunner:
    ...
    def _load_and_prepare_dataset(self):
        ...
        local_longbench_path = Path(__file__).resolve().parents[2] / "datasets" / "LongBench" / ".extracted_longbench" / "data"
        local_longbench_file = local_longbench_path / f"{data_dir}.jsonl" if data_dir else None
        if dataset_name == "longbench" and local_longbench_file and local_longbench_file.exists():
            logger.info(f"Loading local LongBench JSONL: {local_longbench_file}")
            df = load_dataset("json", data_files=str(local_longbench_file), split="train").to_pandas()
            context_prefix = {
            ...
            if data_dir in context_prefix:
                df["context"] = df["context"].map(lambda context: context_prefix[data_dir].format(context=context))
                df["question"] = df["input"].map(lambda input_text: question_template[data_dir].format(input=input_text))
                df["answer_prefix"] = answer_prefix[data_dir]
                df["task"] = data_dir
                df["max_new_tokens"] = max_new_tokens[data_dir]
        else:
